--今日考勤
    ...
    FROM (
        SELECT CONCAT(SUBSTR(start_time,1,5),'_',SUBSTR(end_time,1,5)) as time_gap,grade_name
        FROM school_course_info
        WHERE weekday=dayofweek(date_format({day}, "%y-%m-%d"))
...
--校园安全预警 
    ...
    FROM (
        ...
        WHERE dt>=date_add({day}, interval -15 day) AND dt<={day} AND student_emotion='2'
    ...
    FROM (
        SELECT student_number, student_name, grade_name, count(*) as num
        FROM student_mental_status_ld
        WHERE dt>=date_add({day}, interval -15 day) AND dt<={day} AND student_mental_stat='2'
        GROUP BY student_number, student_name, grade_name
        HAVING num>=6
    )t1 
    GROUP BY grade_name
    ...
    FROM (
        ...
        FROM student_mental_status_ld
        WHERE dt>=date_add({day}, interval -15 day) AND dt<={day} AND student_study_stat='3'
        GROUP BY student_number, student_name, grade_name
        HAVING num>=6
    )t1 
    GROUP BY grade_name
